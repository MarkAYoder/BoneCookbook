...
The recipes in this chapter assume that you are running Linux on your host computer (<<tips_pick_os>>) and are comfortable with using Linux. We continue to assume that you are logged in as +root+ on your Bone.
...
image::images/bglb_0501.png[HDMI Adaptor]
...
==== Solution
For projects that require a host computer, we assume that you are running http://bit.ly/1wXOwkw[Linux Ubuntu 14.04 LTS]. You can be running either a native installation, via a virtual machine such as https://www.virtualbox.org/[VirtualBox], or in the cloud (http://bit.ly/1wu04LI[Microsoft Azure] or http://aws.amazon.com/ec2/[Amazon Elastic Compute Cloud] [EC2], for example).
...
((("superusers")))((("root user")))<<basics_cloud9_IDE>> shows how to run shell commands in the Cloud9 +bash+ tab. However, the Bone has Secure Shell (SSH) enabled right out of the box, so you can easily connect by using the following command to log in as superuser +root+, (note the +#+ at the end of the prompt):
...
((("passwords", "changing")))((("security issues", "passwords")))+root+ has no password. It's best to change both passwords:
...
The letters +crw-rw----+ show that you can't access it as a normal user. However, you _can_ access it if you are part of +dialout+ group. Just add yourself to the group:
...
==== Problem
((("advanced operations", "viewing system messages")))((("FTDI pins")))((("debugging", "low-level")))((("system messages, viewing")))((("debugging", "at boot time")))You want to see the messages that are logged by BeagleBone Black come to life.
...
[[tips_FTDIcable_fig]]
.FTDI cable
image::images/bglb_0502.png[FTDI Cable]
...
.FTDI connector
image::images/bglb_0503.png[FTDI Connector]
...
image::images/bglb_0504.png[Serial Debug Pins]
...
image::images/bglb_0505.png[Create a new remote desktop]
...
image::images/bglb_0506.png[Configuring]
...
image::images/bglb_0507.png[Desktop]
...
image::images/bglb_0508.png[nano]
...
Many other text editors will run on the Bone. +vi+, +vim+, +emacs+, and  even +eclipse+ are all supported. See <<tips_apt-get>> to learn if your favorite is one of them.

=== Using a Graphical Editor

==== Problem
((("advanced operations", "using graphical editors")))((("editors", "graphics-based")))((("graphical editors")))You want to use a graphical (rather than text-based) editor to edit a text file.

==== Solution
((("leafpad editor")))+leafpad+ is a simple graphics-based editor that's already installed on your Bone. You can use it if your host computer supports X-Windows (Mac and Linux). Just run the following commands to open a new file to begin editing in +leafpad+ (<<tips_leafpad_fig>>):
...
[[tips_leafpad_fig]]
.Editing a file with leafpad
image::images/bglb_0509.png[leafpad]
...
image::images/bglb_0510.png[RJ45]
...
((("IP address", "locating")))((("ifconfig command")))To find the IP address, open a terminal window and run the +ifconfig+ command:
...
((("MAC address, locating")))On my university campus, you must register your MAC address before any device will work on the network. The +HWaddr+ field gives the MAC address. For +eth0+, it's +c8:a0:30:b7:f9:6f+.  
...
((("iwconfig command")))Next, run +iwconfig+ to find your adapter's name.  Mine is called +wlan0+, but you might see other other names, such as +ra0+.
...
((("ifconfig command")))If no name appears, try +ifconfig -a+:
...
((("wicd-curses")))Run +wicd-curses+ to display the wireless networks your WiFi device is seeing:
...
In <<tips_wicd_opening_view_fig>>, you see one wireless network, my home wireless. 

[[tips_wicd_opening_view_fig]]
.Using wicd to display available wireless networks
image::images/bglb_0511.png[wicd opening view]

If nothing appears, press P and make sure the Wireless Interface matches the device name you noted from +ifconfig+. If it doesn't match, press the down arrow and enter the device name, as shown in <<tips_wicd_prefs_fig>>.  

[[tips_wicd_prefs_fig]]
.Setting preferences in wicd
image::images/bglb_0512.png[Setting preferences in wicd]

After you enter a name, press F10 to save and then press R to refresh the list. You'll now see the available access points.

Use the down arrow to select which network to use and then press the right arrow  to configure it, as shown in <<tips_wicd_config_fig>>.

[[tips_wicd_config_fig]]
.Configuring a network in wicd
image::images/bglb_0513.png[Configuring a network in wicd]

Press the down arrow until you are near the bottom of the screen and then press Enter. Use the up/down arrows to select the type of authentication (as shown in <<tips_wicd_auth_fig>>) and then press Enter. My home system uses WPA 1/2 (Passphrase) (<<tips_wicd_password_fig>>). 

[[tips_wicd_auth_fig]]
.Configuring authentication in wicd
image::images/bglb_0514.png[Configuring authentication in wicd]

[[tips_wicd_password_fig]]
.Entering password in wicd
image::images/bglb_0515.png[Entering password in wicd]

Press F10 and select the network again; then press C to connect. You'll be connected in a moment, as shown in <<tips_wicd_connected_fig>>.

[[tips_wicd_connected_fig]]
.Connected to a network via wicd
image::images/bglb_0516.png[Connected to a network via wicd]
...
include::images/tips/ipMasquerade.sh[IP masquerade]
...
include::images/tips/setDNS.sh[Set DNS]
...
((("apt-get", "installing packages with")))The easiest way to install more software is to use +apt-get+:
...
The first command lists all the packages that +apt-get+ knows about and sorts them and stores them in _/tmp/list_. The second command shows why you want to put the list in a file. The +wc+ command counts the number of lines, words, and characters in a file. In our case, there are over 32,000 packages from which we can choose! The +less+ command displays the sorted list, one page at a time. Press the space bar to go to the next page. Press Q to quit.  
...
==== Problem
((("software", "removing packages")))((("advanced operations", "removing packages")))((("apt-get", "removing packages installed with")))((("programs", "removing")))You've been playing around and installing all sorts of things with +apt-get+ and now you want to clean things up a bit.

==== Solution
((("remove option")))+apt-get+ has a +remove+ option, so you can run the following command:
...
==== Problem
((("advanced operations", "installing additional Node.js packages")))((("software", "installing additional Node.js packages")))((("Node Package Manager")))You are working with JavaScript and want to build on what others have done.

==== Solution
((("global positioning system (GPS)")))((("National Marine Electronics Association (NMEA)")))Node Package Manager (npm [https://www.npmjs.org/]) is used to install new packages. For example, suppose that you are wiring up a GPS (<<sensors_GPS>>) and want to use the +nmea+ JavaScript package to decode the National Marine Electronics Association (NMEA) data from the GPS. Install the +nmea+ package with the following command:
...
==== Problem
((("advanced operations", "interactions via Python")))((("Python")))You want to use Python on the Bone to talk to the world.

==== Solution
So far, we've focused on using JavaScript and the BoneScript API for interfacing BeagleBone Black to the physical world.  BoneScript provides an easy way to interface the Bone to the world, but there are other programming languages out there.  This recipe provides the steps needed to work with Python, and <<misc_libsoc>> does the same for C.

((("BBIO library")))((("Adafruit BBIO library")))http://www.adafruit.com/[Adafruit] has produced an excellent http://bit.ly/1Mt9kDS[library for the Bone], and it's already installed. Ensure that the BBIO library is up-to-date:
...
((("LEDs", "controlling via Python")))Now that you're ready to go, add the code in <<misc_python_blink>> to a file called _blinkLED.py_.

[role="pagebreak-before"]
[[misc_python_blink]]
.Use Python to blink an LED (blinkLED.py)
====
[source, python]
----

include::images/tips/blinkLED.py[]

----
====

Wire your LED as shown in <<displays_externalLED>> and run it with the following command:
...
Adafruit's http://bit.ly/1C5dkaK[Setting up IO Python Library on BeagleBone Black] does a nice job showing how to use the BBIO library.
...
==== Solution
The C solution isn't as simple as the BoneScript or Python solution (<<using_python>>), but it does work and is much faster. https://github.com/jackmitch/libsoc[libsoc] is a "C library for interfacing with common SoC {system on chip} peripherals through generic kernel interfaces" by http://www.embed.me.uk/[Jack Mitchell]. Here's how to get and install it:
...
This installs the +libsoc+ library in _/usr/local/lib_. Add the code in <<misc_c_blink>> to a file called _blinkLED.c_.
...
include::images/tips/blinkLED.c[]
...
[NOTE]
====
((("GPIO pins", "header pin mapping for libsoc")))((("debugging", "with libsoc library")))In BoneScript and Python, we refer to the GPIO pins by the header position (+P9_14+, for example). In +libsoc+, we refer to them by an internal GPIO number. <<tips_cape_headers_digital>> shows how to map from header position to GPIO pin. For example, +P9_14+ has +GPIO_50+ next to it. That means +libsoc+ needs to use GPIO 50 to interface with +P9_14+.
====

[[tips_cape_headers_digital]]
.Mapping from header pin to internal GPIO number
image::images/bglb_0518.png[cape headers digital]
