...
The first command lists all the packages that +apt+ knows about and sorts them and stores them in _/tmp/list_. The second command shows why you want to put the list in a file. The +wc+ command counts the number of lines, words, and characters in a file. In our case, there are over 32,000 packages from which we can choose! The +less+ command displays the sorted list, one page at a time. Press the space bar to go to the next page. Press Q to quit.  
...
[NOTE]
====
If you have an older image, you might need to install the +libsoc+ library. You can do that with +apt+ if you have network access and the right feeds on your Bone. Install +libsoc2+ for the library and +libsoc-dev+ for the header files.
====
