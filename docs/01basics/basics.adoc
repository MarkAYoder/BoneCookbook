...
bone$ <strong>chmod +x seqLEDs.py</strong> 
...
You need to use the +chmod +x+ command only once. Finally, +./seqLEDs.py+ instructs the python script to run. You will need to press ^C (Ctrl-C) to stop your program.
...
Click the image you want to use and it will download.  The images are som 500M, so it might take a while.
