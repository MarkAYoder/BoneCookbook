...
[NOTE]
====
If the Beagle appears to be running (the LEDs are flashing) but you are having trouble connecting vis +ssh 192.168.7.2+, you can try connecting using the approach shown in <<tips_FTDI>>.
====
...
Notice the NON-RT data have much longer latenices. They may not happen ofen (fewer thatn 10 times in each bin), but they are occuring and may be enough to miss a real-time deadline.
