...
==== Problem
((("input/output (I/O)", "with C and libsoc")))((("C language", "real time I/O with")))((("libsoc library")))You want to use the C language to process inputs in real time, or JavaScript isn't fast enough.

==== Solution
<<realtime_JavaScript>> shows how to control an LED with a pushbutton using JavaScript. This recipe accomplishes the same thing using C and +libsoc+. <<misc_libsoc>> shows how to use C and +libsoc+ to access the GPIO pins.
