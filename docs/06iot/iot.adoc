...
[NOTE]
====
If NODE-RED doesn't appear, run the following in a terminal widow.
++++
<pre data-type="programlisting">
bone$ <strong>sudo systemctl start nodered.service</strong>
</pre>
++++
Wait a bit, then refresh the browser window running NODE-RED.
====
...
[[networking_node_red_inject2_fig]]
.Placing two injects
...
NODE-RED has many other https://flows.nodered.org/[flows and nodes] that can be installed.  You can see them https://flows.nodered.org/[].  Here we'll install *twitter*.
...
[NOTE]
====
If *Manage palette* doesn't appear.  Run the following in a terminal widow.
++++
<pre data-type="programlisting">
bone$ <strong>sudo apt install npm</strong>
bone$ <strong>sudo systemctl restart nodered.service</strong>
</pre>
++++
Wait a bit, then refresh the browser window running NODE-RED.
====
...
.Installing Twitter
image::figures/node-red-install-twitter.png[node-red-install-twitter]
...
. Drag the *twitter in* node to the canvas, as shown in <<networking_node_twitter_fig>>.
...
[start=5]
. Click on the link,  http://developer.twitter.com/en/apps[] shown in <<networking_node_twitter_auth2_fig>>, and follow the steps to set up your twitter app. Copy your *API key* etc. from there.  

[start=7]
. When you're back to Node-RED, click the *Add* button, add your Twitter credentials, enter the hashtags to respond to (<<networking_node_twitter_beagle_fig>>), and then click the *Done* pass:[<span class="keep-together">button</span>].
...
. Scroll down the left panel and drag the *trigger* node to the canvas and wire it (<<networking_node_bbb_out_fig>>).

[[networking_node_bbb_out_fig]]
.Node-RED adding trigger node
image::figures/node-disc-out.png[node-red trigger node]

[start=3]
. Double-click the trigger node and set the time to *1000 ms*.

[[networking_node_bbb_out_setup_fig]]
.Node-RED adding trigger configuration
image::figures/node-disc-out-setup.png[node-red trigger setup]
