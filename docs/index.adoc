include::header.adoc[]

== BeagleBone Cookbook
Mark A. Yoder <Mark.A.Yoder@Rose-Hulman.edu>
v2.0 beta

== Outline

A cookbook for programming Beagles

// * link:notes.html[Notes]

. link:01basics/basics.html[Basics]
  * Done Update Remove
  ** [ ] Introduction
  ** [update which Beagles?] Picking Your Beagle
  ** [vs code] Getting Started, Out of the Box
  ** [ ] Verifying You Have the Latest Version of the OS on Your Bone
  ** [python] Running the BoneScript API Tutorials
  ** [ ] Wiring a Breadboard
  ** [vs code] Editing Code Using the Cloud9 IDE
  ** [vs code] Running JavaScript Applications from the Cloud9 IDE
  ** [remove?] Running Applications Automatically
  ** [update] Finding the Latest Version of the OS for Your Bone
  ** [update] Running the Latest Version of the OS on Your Bone
  ** [update] Updating the OS on Your Bone
  ** [update] Backing Up the Onboard Flash
  ** [update] Updating the Onboard Flash
  
. link:02sensors/sensors.html[Sensors]
  ** [ ] Introduction
  ** [ ] Choosing a Method to Connect Your Sensor
  ** [python/vs code] Input and Run a JavaScript Application for Talking to Sensors
  ** [python] Reading the Status of a Pushbutton or Magnetic Switch (Passive On/Off Sensor)
  ** [python] Reading a Position, Light, or Force Sensor (Variable Resistance Sensor)
  ** [python] Reading a Distance Sensor (Analog or Variable Voltage Sensor)
  ** [ ] Reading a Distance Sensor (Variable Pulse Width Sensor)
  ** [ ] Accurately Reading the Position of a Motor or Dial
  ** [ ] Acquiring Data by Using a Smart Sensor over a Serial Connection
  ** [ ] Measuring a Temperature
  ** [ ] Reading Temperature via a Dallas 1-Wire Device
  ** [remove?] Sensing All Sorts of Things with SensorTag via Bluetooth v4.0
  ** [ ] Playing and Recording Audio
  
. link:03displays/displays.html[Displays]
  ** [ ] Introduction
  ** [python] Toggling an Onboard LED
  ** [python ] Toggling an External LED
  ** [python ] Toggling a High-Voltage External Device
  ** [python ] Fading an External LED
  ** [python ] Writing to an LED Matrix
  ** [remove?] Driving a 5 V Device
  ** [PRU?] Writing to a NeoPixel LED String
  ** [remove?] Using a Nokia 5510 LCD Display
  ** [remove?] Making Your Bone Speak

 . link:04motors/motors.html[Motors]
  ** [ ] Introduction
  ** [python ] Controlling a Servo Motor
  ** [python ] Controlling the Speed of a DC Motor
  ** [parts?] Controlling the Speed and Direction of a DC Motor
  ** [parts? ] Driving a Bipolar Stepper Motor
  ** [parts? ] Driving a Unipolar Stepper Motor

. link:05tips/tips.html[Beyond Basics]
  ** [ ] Introduction
  ** [hdmi ] Running Your Bone Standalone
  ** [ ] Selecting an OS for Your Development Host Computer
  ** [ ] Getting to the Command Shell via SSH
  ** [ ] Getting to the Command Shell via the Virtual Serial Port
  ** [ftdi ] Viewing and Debugging the Kernel and u-boot Messages at Boot Time
  ** [ ] Verifying You Have the Latest Version of the OS on Your Bone from the Shell
  ** [update] Controlling the Bone Remotely with VNC
  ** [ ] Learning Typical GNU/Linux Commands
  ** [ ] Editing a Text File from the GNU/Linux Command Shell
  ** [remove? ] Using a Graphical Editor
  ** [update to ip ] Establishing an Ethernet-Based Internet Connection
  ** [update to? ] Establishing a WiFi-Based Internet Connection
  ** [update IP Masquerading ] Sharing the Host's Internet Connection over USB
  ** [update add install ] Setting Up a Firewall
  ** [update ] Installing Additional Packages from the Debian Package Feed
  ** [ ] Removing Packages Installed with apt
  ** [update /dev/mmcblk1p1 ] Copying Files Between the Onboard Flash and the MicroSD Card
  ** [update What else to remove ] Freeing Space on the Onboard Flash or MicroSD Card
  ** [remove ] Installing Additional Node.js Packages
  ** [remove, add rust? ] Using Python to Interact with the Physical World
  ** [libsoc? ] Using C to Interact with the Physical World

. link:06iot/iot.html[Internet of Things]
  ** [ ] Introduction
  ** [ ] Accessing Your Host Computer's Files on the Bone
  ** [update how? ] Serving Web Pages from the Bone
  ** [replace with flask?] Interacting with the Bone via a Web Browser
  ** [flask?] Displaying GPIO Status in a Web Browser
  ** [remove ] Continuously Displaying the GPIO Value via jsfiddle
  ** [remove] Continuously Displaying the GPIO Value
  ** [update plot via sheets? ] Plotting Data
  ** [update ] Sending an Email
  ** [update ] Sending an SMS Message
  ** [update ] Displaying the Current Weather Conditions
  ** [update ] Sending and Receiving Tweets
  ** [update, remove?] Wiring the IoT with Node-RED
  ** [remove?] Serving Web Pages from the Bone by Using Apache
  ** [hardware?] Communicating over a Serial Connection to an Arduino or LaunchPad

. link:07kernel/kernel.html[The Kernel]
  ** [ ] Introduction
  ** [ ] Updating the Kernel
  ** [ ] Building and Installing Kernel Modules
  ** [update ] Controlling LEDs by Using SYSFS Entries
  ** [add gpiod? ] Controlling GPIOs by Using SYSFS Entries
  ** [update? ] Compiling the Kernel
  ** [update] Using the Installed Cross Compiler
  ** [ ] Applying Patches
  ** [ ] Creating Your Own Patch File

. link:08realtime/realtime.html[Real-Time I/O]
  ** [ ] Introduction
  ** [python ] I/O with BoneScript
  ** [libsoc? ] I/O with C and libsoc
  ** [ ] I/O with devmem2
  ** [ ] I/O with C and mmap()
  ** [replace with preemtn RT?] Modifying the Linux Kernel to Use Xenomai
  ** [remove ] I/O with PRU Speak

. link:09capes/capes.html[Capes]
  ** [ ] Introduction
  ** [remove? ] Using a Seven-Inch LCD Cape
  ** [parts?] Using a 128 x 128-Pixel LCD Cape
  ** [ ] Connecting Multiple Capes
  ** [jason ] Moving from a Breadboard to a Protoboard
  ** [ ] Creating a Prototype Schematic
  ** [python ] Verifying Your Cape Design
  ** [ ] Laying Out Your Cape PCB
  ** [ ] Migrating a Fritzing Schematic to Another Tool
  ** [ ] Producing a Prototype
  ** [ ] Creating Contents for Your Cape Configuration EEPROM
  ** [ ] Putting Your Cape Design into Production

. link:10parts/parts.html[Parts and Supppliers]
  ** [ ] Parts
  ** [ ] Prototyping Equipment
  ** [ ] Resistors
  ** [ ] Transistors and Diodes
  ** [ ] Integrated Circuits
  ** [ ] Opto-Electronics
  ** [ ] Capes
  ** [ ] Miscellaneous